...
- **✍️ ChampPDF Sign (e-signature)** — send an approved template (Mutual NDA v1 ships) for electronic signature: tracked link, email verification code, read gate, typed or drawn signature, PAdES-sealed PDF with a certificate of completion and a hash-chained audit trail. `/sign` for senders, `/s/<token>` for signers. Engine-agnostic via a `SigningProvider` seam (native by default, Documenso optional). See [`docs/sign/README.md`](./docs/sign/README.md).
