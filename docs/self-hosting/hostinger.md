...
[Hostinger](https://hostinger.com) is a popular shared hosting provider. This guide covers deploying ChamPDF to Hostinger's shared hosting.
...
RewriteCond %{HTTP_HOST} ^champdf\.com [NC]
RewriteRule ^(.*)$ https://www.champdf.com/$1 [L,R=301]
...
Make sure the `.html` extension rule and language routes are correctly configured. ChamPDF uses static HTML files, not SPA routing:
