...
export function registerAllTools(server: McpServer): void {
  // (server is created by the caller — this file just registers tools on it)
    ...
    async () => {
      try {
        ...
        const scopes = body.scopes?.length
          ...
          : '';
        return ok(
          `ChamPDF key #${body.id} "${body.label}": ${body.requests_used} of ${body.monthly_quota} requests used this month (bucket ${body.month_bucket}).${scopes}`
        );
      } catch (err) {
        return fail(err);
      }
    ...
    'champdf_inpaint_image',
    {
      ...
      inputSchema: {
        ...
        radius: z
          ...
          .default(5)
          .describe(
            'Inpaint radius in pixels (only used by the OpenCV fallback).'
          ),
      },
    },
  ...
  server.registerTool(
    'champdf_download_video',
    {
      ...
      inputSchema: {
        url: z.string().url().describe('Public YouTube or Instagram URL.'),
        format: z
          .enum(['mp4', 'mp3'])
          .default('mp4')
    ...
    'champdf_remove_video_logo',
    {
      ...
      inputSchema: {
        input_path: z
          .string()
          .describe(
            'Absolute path to the source video (mp4, mov, webm, or avi).'
          ),
        output_path: z
          .string()
        ...
        logo_scale: z
          ...
          .default(1.0)
          .describe(
            'Size multiplier for the replacement logo (0.5–2.0; 1.0 ≈ 120px wide).'
          ),
      },
    },
    async ({
  ...
  // Document workflow tools (merge/split/sign/convert/OCR/...) live in
  // document-tools.ts to keep this file manageable.
