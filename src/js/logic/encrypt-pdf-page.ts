...
async function encryptPdf() {
  ...
  try {
    ...
    // Permission restrictions only take effect when a distinct owner
    // password gates them. Map the checkboxes to qpdf flags; an unchecked
    // box blocks that action for anyone opening with the user password.
    if (hasDistinctOwnerPassword) {
      const allowPrint =
        (document.getElementById('perm-print') as HTMLInputElement)?.checked ??
        true;
      const allowCopy =
        (document.getElementById('perm-copy') as HTMLInputElement)?.checked ??
        true;
      const allowModify =
        (document.getElementById('perm-modify') as HTMLInputElement)?.checked ??
        true;
      const allowAnnotate =
        (document.getElementById('perm-annotate') as HTMLInputElement)
          ?.checked ?? true;
