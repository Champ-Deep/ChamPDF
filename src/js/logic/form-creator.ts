...
function renderGrid() {
  ...
  if (gridV > 0) {
    ...
    for (let i = 0; i <= gridV; i++) {
      ...
      line.className =
        'absolute top-0 bottom-0 border-l-2 border-red-500 opacity-60';
  ...
  if (gridH > 0) {
    ...
    for (let i = 0; i <= gridH; i++) {
      const line = document.createElement('div');
      line.className =
        'absolute left-0 right-0 border-t-2 border-red-500 opacity-60';
...
toolItems.forEach((item) => {
  ...
  item.addEventListener('click', () => {
    ...
    if (selectedToolType === type) {
      ...
      item.classList.remove('ring-2', 'ring-orange-400', 'bg-red-600');
      canvas.style.cursor = 'default';
    } else {
      // Deselect previous tool
      if (selectedToolType) {
        toolItems.forEach((t) =>
          t.classList.remove('ring-2', 'ring-orange-400', 'bg-red-600')
        );
      }
      // Select new tool
      selectedToolType = type;
      item.classList.add('ring-2', 'ring-orange-400', 'bg-red-600');
...
function renderField(field: FormField): void {
  ...
  fieldContainer.className =
    'field-container relative border-2 border-orange-200 group-hover:border-dashed group-hover:border-orange-300 bg-orange-50/30 rounded transition-all';
  ...
  handles.forEach((pos) => {
    ...
    handle.className = `absolute w-2.5 h-2.5 bg-white border border-red-600 z-10 cursor-${pos}-resize resize-handle hidden`; // Added hidden class
  ...
  if (field.type === 'text') {
    specificProps = `
        <div>
            ...
            <input type="text" id="propValue" value="${field.defaultValue}" ${field.combCells > 0 ? `maxlength="${field.combCells}"` : field.maxLength > 0 ? `maxlength="${field.maxLength}"` : ''} class="w-full bg-gray-600 border border-gray-500 text-white rounded px-2 py-1 text-sm focus:ring-red-500 focus:border-red-500">
        </div>
        <div>
            <label class="block text-xs font-semibold text-gray-300 mb-1">Max Length (0 for unlimited)</label>
            <input type="number" id="propMaxLength" value="${field.maxLength}" min="0" ${field.combCells > 0 ? 'disabled' : ''} class="w-full bg-gray-600 border border-gray-500 text-white rounded px-2 py-1 text-sm focus:ring-red-500 focus:border-red-500 disabled:opacity-50">
        </div>
        <div>
            <label class="block text-xs font-semibold text-gray-300 mb-1">Divide into boxes (0 to disable)</label>
            <input type="number" id="propComb" value="${field.combCells}" min="0" class="w-full bg-gray-600 border border-gray-500 text-white rounded px-2 py-1 text-sm focus:ring-red-500 focus:border-red-500">
        </div>
        <div>
            <label class="block text-xs font-semibold text-gray-300 mb-1">Font Size</label>
            <input type="number" id="propFontSize" value="${field.fontSize}" min="8" max="72" class="w-full bg-gray-600 border border-gray-500 text-white rounded px-2 py-1 text-sm focus:ring-red-500 focus:border-red-500">
        </div>
        <div>
        ...
        <div>
            ...
            <select id="propAlignment" class="w-full bg-gray-600 border border-gray-500 text-white rounded px-2 py-1 text-sm focus:ring-red-500 focus:border-red-500">
        ...
        <div class="flex items-center justify-between bg-gray-600 p-2 rounded mt-2">
            ...
            <button id="propMultilineBtn" class="w-12 h-6 rounded-full transition-colors duration-200 focus:outline-none ${field.multiline ? 'bg-red-600' : 'bg-gray-500'} relative">
    ...
    specificProps = `
        <div class="flex items-center justify-between bg-gray-600 p-2 rounded">
            ...
            <button id="propCheckedBtn" class="w-12 h-6 rounded-full transition-colors duration-200 focus:outline-none ${field.checked ? 'bg-red-600' : 'bg-gray-500'} relative">
    ...
    specificProps = `
        <div>
            ...
            <input type="text" id="propGroupName" value="${field.groupName}" class="w-full bg-gray-600 border border-gray-500 text-white rounded px-2 py-1 text-sm focus:ring-red-500 focus:border-red-500">
        </div>
        <div>
            <label class="block text-xs font-semibold text-gray-300 mb-1">Export Value</label>
            <input type="text" id="propExportValue" value="${field.exportValue}" class="w-full bg-gray-600 border border-gray-500 text-white rounded px-2 py-1 text-sm focus:ring-red-500 focus:border-red-500">
        </div>
        <div class="flex items-center justify-between bg-gray-600 p-2 rounded mt-2">
            <label for="propChecked" class="text-xs font-semibold text-gray-300">Checked State</label>
            <button id="propCheckedBtn" class="w-12 h-6 rounded-full transition-colors duration-200 focus:outline-none ${field.checked ? 'bg-red-600' : 'bg-gray-500'} relative">
    ...
    specificProps = `
        <div>
            ...
            <textarea id="propOptions" class="w-full bg-gray-600 border border-gray-500 text-white rounded px-2 py-1 text-sm focus:ring-red-500 focus:border-red-500 h-24">${field.options?.join('\n')}</textarea>
        </div>
        <div>
            <label class="block text-xs font-semibold text-gray-300 mb-1">Selected Option</label>
            <select id="propSelectedOption" class="w-full bg-gray-600 border border-gray-500 text-white rounded px-2 py-1 text-sm focus:ring-red-500 focus:border-red-500">
    ...
    specificProps = `
        <div>
            ...
            <input type="text" id="propLabel" value="${field.label}" class="w-full bg-gray-600 border border-gray-500 text-white rounded px-2 py-1 text-sm focus:ring-red-500 focus:border-red-500">
        </div>
        <div>
            <label class="block text-xs font-semibold text-gray-300 mb-1">Action</label>
            <select id="propAction" class="w-full bg-gray-600 border border-gray-500 text-white rounded px-2 py-1 text-sm focus:ring-red-500 focus:border-red-500">
        ...
        <div id="propUrlContainer" class="${field.action === 'url' ? '' : 'hidden'}">
            ...
            <input type="text" id="propActionUrl" value="${field.actionUrl || ''}" placeholder="https://example.com" class="w-full bg-gray-600 border border-gray-500 text-white rounded px-2 py-1 text-sm focus:ring-red-500 focus:border-red-500">
        </div>
        <div id="propJsContainer" class="${field.action === 'js' ? '' : 'hidden'}">
            ...
            <textarea id="propJsScript" class="w-full bg-gray-600 border border-gray-500 text-white rounded px-2 py-1 text-sm focus:ring-red-500 focus:border-red-500 h-24 font-mono">${field.jsScript || ''}</textarea>
        </div>
        <div id="propShowHideContainer" class="${field.action === 'showHide' ? '' : 'hidden'}">
            <div class="mb-2">
                ...
                <select id="propTargetField" class="w-full bg-gray-600 border border-gray-500 text-white rounded px-2 py-1 text-sm focus:ring-red-500 focus:border-red-500">
            ...
            <div>
                ...
                <select id="propVisibilityAction" class="w-full bg-gray-600 border border-gray-500 text-white rounded px-2 py-1 text-sm focus:ring-red-500 focus:border-red-500">
    ...
    specificProps = `
        <div>
            ...
            <select id="propDateFormat" class="w-full bg-gray-600 border border-gray-500 text-white rounded px-2 py-1 text-sm focus:ring-red-500 focus:border-red-500">
        ...
        <div id="customFormatContainer" class="${isCustom ? '' : 'hidden'} mt-2">
            ...
            <input type="text" id="propCustomFormat" value="${isCustom ? field.dateFormat : ''}" placeholder="e.g. dd/mm/yyyy HH:MM:ss" class="w-full bg-gray-600 border border-gray-500 text-white rounded px-2 py-1 text-sm focus:ring-red-500 focus:border-red-500">
        </div>
  ...
  propertiesPanel.innerHTML = `
    <div class="space-y-3">
      <div>
        ...
        <input type="text" id="propName" value="${field.name}" class="w-full bg-gray-600 border border-gray-500 text-white rounded px-2 py-1 text-sm focus:ring-red-500 focus:border-red-500">
      ...
      <div>
        ...
        <select id="existingGroups" class="w-full bg-gray-600 border border-gray-500 text-white rounded px-2 py-1 text-sm focus:ring-red-500 focus:border-red-500">
      ...
      <div>
        ...
        <input type="text" id="propTooltip" value="${field.tooltip}" placeholder="Description for screen readers" class="w-full bg-gray-600 border border-gray-500 text-white rounded px-2 py-1 text-sm focus:ring-red-500 focus:border-red-500">
      </div>
  ...
  if (field.type === 'text') {
    ...
    if (propMultilineBtn) {
      propMultilineBtn.addEventListener('click', () => {
        ...
        if (field.multiline) {
          ...
          propMultilineBtn.classList.add('bg-red-600');
          span?.classList.remove('translate-x-0');
          span?.classList.add('translate-x-6');
        } else {
          propMultilineBtn.classList.remove('bg-red-600');
    ...
    propCheckedBtn.addEventListener('click', () => {
      ...
      if (field.checked) {
        ...
        propCheckedBtn.classList.add('bg-red-600');
        span?.classList.remove('translate-x-0');
        span?.classList.add('translate-x-6');
      } else {
        propCheckedBtn.classList.remove('bg-red-600');
...
dropZone.addEventListener('dragover', (e) => {
  e.preventDefault();
  dropZone.classList.add('border-red-500', 'bg-gray-600');
});

dropZone.addEventListener('dragleave', () => {
  dropZone.classList.remove('border-red-500', 'bg-gray-600');
});
